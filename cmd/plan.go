...
to quickly create a Cobra application.`,
	...
	planCmd.PersistentFlags().StringVarP(&config.StateBucket, "state-bucket", "", "cloud-platform-terraform-state", "State bucket where terraform state file is stored")
	planCmd.PersistentFlags().StringVarP(&config.StateKeyPrefix, "state-key-prefix", "", "cloud-platform-environments/", "State buucket key prefix location")

	planCmd.PersistentFlags().StringVarP(&config.StateLockTable, "terraform-state-lock-table", "", "cloud-platform-environments-terraform-lock", "DynamoDB table to store the state md5")

	planCmd.PersistentFlags().StringVarP(&config.StateRegion, "state-region", "", "eu-west-1", "AWS Region")

	planCmd.PersistentFlags().StringVarP(&config.Cluster, "cluster", "", "cp-2004-1705.cloud-platform.service.justice.gov.uk", "Cluster to which the manifest will be applied")

	planCmd.PersistentFlags().StringVarP(&config.RepoPath, "repo-path", "", "namespaces/cp-2004-1705.cloud-platform.service.justice.gov.uk", "Repository folder path where the namespace manifest are")
	planCmd.PersistentFlags().IntVarP(&config.NumRoutines, "routines", "", 2, "Num of go routines to split the folder into")

	planCmd.PersistentFlags().StringVarP(&config.Folder, "namespace", "", "ns-test-dev", "Name of the folder to do the plan")
